...
SELECT cloudsync_init('products', 'CLS', 1) AS _init_products \gset
...
SELECT cloudsync_init('users', 'CLS', 1) AS _init_users_public \gset

-- Attempt to init again in same connection (should FAIL)
DO $$
BEGIN
    PERFORM cloudsync_init('users', 'CLS', 1);
...
SELECT cloudsync_init('orders', 'CLS', 1) AS _init_orders \gset
...
SELECT cloudsync_init('products_b', 'CLS', 1) AS _init_products_b \gset
...
SELECT cloudsync_init('orders_sales', 'CLS', 1) AS _init_orders_sales \gset

SELECT cloudsync_set_schema('analytics') AS _set_analytics \gset
SELECT cloudsync_init('reports', 'CLS', 1) AS _init_reports \gset
...
SELECT cloudsync_init('test_table', 'CLS', 1) AS _init_test_table \gset
